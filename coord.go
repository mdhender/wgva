...
// they are what keeps the extreme negative value of Component out of a Coord,
// and with it out of negation, absolute value, and the int64 round trip.
...
// rather than against the Component type's own range, which is wider on the
// negative side. See DESIGN.md 4.1.
...
// float32: at the shipping component width, float32 spacing at the map's far
// corner is about 300 hexes. See DESIGN.md 7.
...
// abs64 is total for every value this package produces, because DESIGN.md 4.1
// excludes math.MinInt64's Component analogue from the canonical domain.
...
// components at ±(2N+1), outside Component range at either width, which is what
// the int64 intermediate rule of DESIGN.md 4 protects.
//
